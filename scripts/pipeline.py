...
POTHOLE_WINDOWS = {
    ...
    'B': (2.0, 0.5), # 2.0 before, 0.5 after
    'C': (1.25, 0.25), # 1.25 before, 0.25 after
}
...
def extract_windows(accel, clean_events, ph_pre, ph_post):
    ...
    for _, row in clean_events.iterrows(): # sort through cleaned events csv, and extract windows
        ...
        # create event IDs
        id_event = f"{test_num.replace(' ', '_')}_{type_event}_{pd.Timestamp(wall_time).strftime('%H%M%S%f')}"
        # create normal_windows
        total_window = ph_pre + ph_post
        normal_window = total_window / 2

        # define which window we are using, the given pothole window or the standard normal window
        pre, post = (ph_pre, ph_post) if type_event == 'POTHOLE' else (normal_window, normal_window)
        ...
        # check if window length is too small, skip if not long enough
        expected_samples = int((pre + post) * 100)
        if len(block) < expected_samples * 0.9:
            continue
...
if __name__ == "__main__":
    ...
    folders = sorted([d for d in os.listdir('data/input_data') if d.startswith('test_')])
    folders = [os.path.join('data/input_data', f) for f in folders]
    ...
    output_dir = Path('data/intermediary_data')
    ...
    for win_key, (ph_pre, ph_post) in POTHOLE_WINDOWS.items(): # loop through pothole windows
        for thresh_key, thresh_sec in CONTAM_THRESHOLDS.items(): # loop through contamination thresholds
            ...
            if len(windows) > 0: # check if windows are successfully populated
                windows.to_csv(output_dir / f"supervised_{cfg}.csv", index=False)
