...
# use directory to find output files
plot_dir = Path('output/plots')
plot_dir.mkdir(parents=True, exist_ok=True)

# find all supervised csvs
supervised_files = sorted(Path('output').glob('supervised_*.csv'))
...
for csv_path in supervised_files:
    ...
    for road_type in ['POTHOLE', 'NORMAL']:
        ...
        fig.tight_layout()
        fig.savefig(plot_dir / f"{road_type.lower()}_{cfg_name}.png", dpi=120)
        plt.close(fig)
